...
@Service
public class MotorAService {

    private static final Logger logger = LoggerFactory.getLogger(MotorAService.class);
    ...
    private void loadRulesFromYaml() throws Exception {
        ClassPathResource resource = new ClassPathResource("rules/motor/motor-a-rules.yml");
        try (Reader reader = new InputStreamReader(resource.getInputStream(), StandardCharsets.UTF_8)) {
            MVELRuleFactory ruleFactory = new MVELRuleFactory(new YamlRuleDefinitionReader());
            rules = ruleFactory.createRules(reader);
            logger.info("Loaded {} rules from motor-a-rules.yml", rules.size());
    ...
    public MotorAResponse getMotor(MotorARequest request) {
        if (request == null) {
            return MotorAResponse.of("NA", "Request is null");
        }

        logger.info("Evaluating motor for: speed={}, q={}, tl={}, wgtCarDecorationTotal={}", 
            request.getSpeed(), request.getQ(), request.getTl(), request.getWgtCarDecorationTotal());
        ...
        logger.info("Motor result: {} -> {}", result.matchedRule, result.result);

        return MotorAResponse.of(result.result, result.matchedRule);
    }
