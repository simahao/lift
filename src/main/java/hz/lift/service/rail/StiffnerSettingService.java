...
@Service
public class StiffnerSettingService {

    private static final Logger logger = LoggerFactory.getLogger(StiffnerSettingService.class);
    ...
    private void loadRulesFromYaml() throws Exception {
        ClassPathResource resource = new ClassPathResource("rules/rail/mrl-stiffner-setting-rules.yml");
        try (Reader reader = new InputStreamReader(resource.getInputStream(), StandardCharsets.UTF_8)) {
            MVELRuleFactory ruleFactory = new MVELRuleFactory(new YamlRuleDefinitionReader());
            rules = ruleFactory.createRules(reader);
            logger.info("Loaded {} rules from mrl-stiffner-setting-rules.yml", rules.size());
    ...
    public StiffnerSettingResponse getSetting(StiffnerSettingRequest request) {
        if (request == null) {
            return StiffnerSettingResponse.of("NA", "Request is null");
        }

        logger.info("Evaluating rules for: typProductModel={}, wgtRatedLoadQ={}, typMacHoisting={}, typCarGr={}", 
            request.getTypProductModel(), request.getWgtRatedLoadQ(), request.getTypMacHoisting(), 
            request.getTypCarGr());

        Facts facts = new Facts();
        facts.put("request", request);
        
        StiffnerResult result = new StiffnerResult();
        ...
        return StiffnerSettingResponse.of(result.result, result.matchedRule);
    }

    public static class StiffnerResult {
