...
@Service
public class XSettingService {

    private static final Logger logger = LoggerFactory.getLogger(XSettingService.class);
    ...
    private void loadRulesFromYaml() throws Exception {
        ClassPathResource resource = new ClassPathResource("rules/rail/x-setting-rules.yml");
        try (Reader reader = new InputStreamReader(resource.getInputStream(), StandardCharsets.UTF_8)) {
            MVELRuleFactory ruleFactory = new MVELRuleFactory(new YamlRuleDefinitionReader());
            rules = ruleFactory.createRules(reader);
            logger.info("Loaded {} rules from x-setting-rules.yml", rules.size());
    ...
    public XSettingResponse getSetting(XSettingRequest request) {
        if (request == null) {
            return XSettingResponse.of("NA", "Request is null");
        }

        logger.info("Evaluating rules for: qtyGrCwtTopBLength={}, typCwtGr={}", 
        ...
        return XSettingResponse.of(result.result, result.matchedRule);
    }
