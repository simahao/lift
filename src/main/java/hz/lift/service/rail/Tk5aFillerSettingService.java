...
@Service
public class Tk5aFillerSettingService {

    private static final Logger logger = LoggerFactory.getLogger(Tk5aFillerSettingService.class);
    ...
    private void loadRulesFromYaml() throws Exception {
        ClassPathResource resource = new ClassPathResource("rules/rail/tk5a-filler-setting-rules.yml");
        try (Reader reader = new InputStreamReader(resource.getInputStream(), StandardCharsets.UTF_8)) {
            MVELRuleFactory ruleFactory = new MVELRuleFactory(new YamlRuleDefinitionReader());
            rules = ruleFactory.createRules(reader);
            logger.info("Loaded {} rules from tk5a-filler-setting-rules.yml", rules.size());
    ...
    public Tk5aFillerSettingResponse getSetting(Tk5aFillerSettingRequest request) {
        if (request == null) {
            return Tk5aFillerSettingResponse.of("NA", "Request is null");
        }

        logger.info("Evaluating rules for: typCwtGr={}, valRatedSpeed={}", 
            request.getTypCwtGr(), request.getValRatedSpeed());

        Facts facts = new Facts();
        facts.put("request", request);
        
        FillerResult result = new FillerResult();
        ...
        return Tk5aFillerSettingResponse.of(result.result, result.matchedRule);
    }

    public static class FillerResult {
